  ...
  return profile.topics
    .map((topic, index) => {
      ...
      return {
        ...
        reason:
          reasons[0] ??
          (locale === "kz" ? "тақырыпты қосымша бекіту қажет" : "тему нужно дополнительно закрепить"),
        resource: {
...
function generateRecommendations(
  ...
  if (locale === "kz") {
    if (topGap) {
      items.push(`${topGap.title} бойынша 2 қысқа қайталау слоты мен 12 тапсырманы ${predictedAssessmentLabel} дейін орындаңыз.`);
    }
    if (resourceTitle) {
      items.push(`"${resourceTitle}" материалын қарап, сол күні 5 тапсырмамен бекітіңіз.`);
    }
    if ((attendance?.missingWithoutReason ?? 0) > 0) {
      items.push("Себепсіз босатуларды нөлге түсіріп, бір аптаға қатысу жоспарын бекітіңіз.");
    } else if (trend === TrendLabel.declining || trend === TrendLabel.critical_decline) {
      items.push("Мұғаліммен қысқа консультация жоспарлап, апталық қайталау ретін қайта жинаңыз.");
    } else if (secondGap) {
      items.push(`${secondGap.title} бойынша one-page summary жасап, апта соңында өзіңізді тексеріңіз.`);
    }
  } else {
    if (topGap) {
      items.push(`Закрой тему "${topGap.title}" через 2 коротких слота и 12 задач до ${predictedAssessmentLabel}.`);
    }
    if (resourceTitle) {
      items.push(`Посмотри материал "${resourceTitle}" и закрепи его 5 заданиями в тот же день.`);
    }
    if ((attendance?.missingWithoutReason ?? 0) > 0) {
  ...
  while (items.length < 3) {
    items.push(
      locale === "kz"
        ? "Апта соңында mini-check жасап, қате кеткен тапсырмаларды қайта орындаңыз."
        : "В конце недели сделай mini-check и повторно реши задания, где были ошибки."
    );
  }
...
function explainRisk(
  ...
  if (trend === TrendLabel.declining || trend === TrendLabel.critical_decline) {
    reasons.push(locale === "kz" ? "теріс тренд байқалады" : "наблюдается негативный тренд");
  }
  if ((attendance?.totalMissCount ?? 0) > 2) {
  ...
  if (topGap) {
    reasons.push(locale === "kz" ? `негізгі олқылық: ${topGap.title.toLowerCase()}` : `ключевой пробел: ${topGap.title.toLowerCase()}`);
  }
  if (!reasons.length) {
    reasons.push(locale === "kz" ? "нәтиже жұмыс аймағында" : "результат находится в рабочей зоне");
  }
  const rawScore = Math.round(
    breakdown.baseRisk +
      breakdown.trendPenalty +
      breakdown.attendancePenalty +
      breakdown.unexcusedPenalty +
      breakdown.stabilityBoost
  );
  return locale === "kz"
    ? `${subjectName}: тәуекел ${rawScore}%. Себептері: ${reasons.join(", ")}.`
    : `${subjectName}: риск ${rawScore}%. Причины: ${reasons.join(", ")}.`;
}
...
export function calculateSubjectRisk(input: SubjectAnalyticsInput, locale: Locale): SubjectRiskResult {
  ...
  return {
    ...
    tutorHeadline:
      locale === "kz"
        ? `${subjectName} бойынша ${Math.round(probabilityFail)}% тәуекел бар: ${predictedAssessmentLabel} алдында ${topGap ? `"${topGap.title}"` : "әлсіз тақырып"} шешуші болады.`
        : `По предмету ${subjectName} риск ${Math.round(probabilityFail)}%: к ${predictedAssessmentLabel} главным ограничением станет ${topGap ? `"${topGap.title}"` : "слабая тема"}.`
  };
}
...
export function buildWeeklySummary(locale: Locale, input: { fullName: string; subjectRisks: SubjectRiskResult[]; totalMisses: number }) {
  ...
  return locale === "kz"
    ? {
        ...
        strongSides: strongest
          ? [
              `Күшті бағыт: ${strongest.subjectName} (${Math.round(strongest.averageScore ?? 0)}%).`,
              `${strongest.subjectName} пәнінде динамика ${strongest.trendLabel.toLowerCase()}.`
            ]
          : ["Айқын күшті пән әзірге жеткіліксіз."],
        problemZones: weakest
          ? [
              `Назар аймағы: ${weakest.subjectName} (${Math.round(weakest.riskScore)}% тәуекел).`,
              topGap ? `Негізгі тақырыптық олқылық: ${topGap.title}.` : `${weakest.subjectName} бойынша қайталау қажет.`
            ]
          : ["Критикалық тәуекел аймағы анықталмады."],
        misses: input.totalMisses,
        advice:
          weakest?.riskScore && weakest.riskScore >= 50
            ? `${weakest.predictedAssessmentLabel} дейін ${weakest.subjectName} бойынша 2 қысқа дайындық слоты мен 1 консультация жоспарлаған дұрыс.`
            : "Қазіргі режимді сақтап, апталық прогресс пен қатысуды бақылауды жалғастырыңыз."
      }
    : {
        student: input.fullName,
        strongSides: strongest
          ? [
              `Сильная сторона: ${strongest.subjectName} (${Math.round(strongest.averageScore ?? 0)}%).`,
              `По предмету ${strongest.subjectName} динамика: ${strongest.trendLabel.toLowerCase()}.`
            ]
          : ["Явно выраженного сильного предмета пока недостаточно."],
        problemZones: weakest
          ? [
              `Зона внимания: ${weakest.subjectName} (${Math.round(weakest.riskScore)}% риска).`,
              topGap ? `Ключевой тематический пробел: ${topGap.title}.` : `По предмету ${weakest.subjectName} нужен повтор.`
            ]
          : ["Критических зон не выявлено."],
        misses: input.totalMisses,
        advice:
          weakest?.riskScore && weakest.riskScore >= 50
            ? `До ${weakest.predictedAssessmentLabel} стоит зафиксировать по ${weakest.subjectName} 2 коротких слота повторения и 1 консультацию.`
            : "Сохраняйте текущий режим и отслеживайте прогресс по неделям."
      };
}

export function buildTeacherReport(
  locale: Locale,
  input: { className: string; items: Array<{ studentName: string; highestRisk: SubjectRiskResult; avgScore: number | null; misses: number }> }
) {
  ...
  const riskStudents = input.items
    ...
    .map((item) => `${item.studentName} (${item.highestRisk.subjectName}${item.highestRisk.knowledgeGaps[0] ? `: ${item.highestRisk.knowledgeGaps[0].title}` : ""})`);
  const attendanceStudents = input.items.filter((item) => item.misses >= 3).map((item) => item.studentName);
  const priority = input.items
    .filter((item) => item.highestRisk.riskScore >= 65)
    .slice(0, 2)
    .map((item) => `${item.studentName} - ${item.highestRisk.tutorHeadline}`);

  if (locale === "kz") {
    return [
      `${input.className} сыныбы: ${strongStudents.length} оқушы күшті аймақта, ${riskStudents.length} оқушы ерте араласуды қажет етеді.`,
      strongStudents.length ? `Көшбасшылар: ${strongStudents.join(", ")}.` : "Айқын көшбасшылар тобы байқалмады.",
      riskStudents.length ? `Тәуекел тобы: ${riskStudents.join(", ")}.` : "Критикалық тәуекел тобы жоқ.",
      attendanceStudents.length ? `Қатысу бақылауы қажет: ${attendanceStudents.join(", ")}.` : "Қатысу бойынша елеулі мәселе анықталмады.",
      priority.length ? `Приоритетті араласу: ${priority.join(" ")}` : "Нақты араласу қажет ететін оқушы саны шектеулі."
    ].join(" ");
  }

  return [
    `Класс ${input.className}: ${strongStudents.length} учеников в сильной зоне, ${riskStudents.length} требуют раннего вмешательства.`,
    strongStudents.length ? `Лидеры: ${strongStudents.join(", ")}.` : "Явной группы лидеров нет.",
    ...
    priority.length ? `Приоритетное вмешательство: ${priority.join(" ")}` : "Точечное вмешательство требуется ограниченному числу учеников."
  ].join(" ");
}
